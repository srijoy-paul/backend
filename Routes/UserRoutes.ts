...
router.post("/signup", jwtCheck, async (req: any, res: any) => {
  try {
    ...
    const existingUser = await pool.query(
      "SELECT * FROM userinfo WHERE auth0id=$1",
      [auth0id]
    );
    ...
    if (existingUser.rows.length !== 0) {
      return res
        .status(200)
        .json({ status: "ok", message: "User already exists." });
    }
    const newUser = await pool.query(
      "INSERT INTO userinfo(auth0id,email) VALUES($1,$2) RETURNING *",
      [auth0id, email]
    );
    ...
    return res
      ...
      .json({
        status: "error",
        message: "Error while creating your user account.",
      });
  }
});
  ...
  async (req: any, res: any) => {
    if (req.body !== null) {
      ...
      const updateUserDetails = await pool.query(
        "UPDATE userinfo SET name=$1,addressline1=$2,city=$3,country=$4 WHERE auth0id=$5 RETURNING *",
        [
          name,
          addressline1,
      ...
      return res
        ...
        .json({
          ...
          details: { name, addressline1, country, city },
        });
    }
  }
);
...
router.get("/getUserInfo", jwtCheck, parseJWT, async (req: any, res: any) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Successfully fetched!", userinfo: existingUser });
  } catch (error) {
    console.log(error);
