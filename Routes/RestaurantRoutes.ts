...
router.get(
  ...
  async (req: any, res: any) => {
    try {
      ...
      const availableRestaurants = await pool.query(
        `SELECT * FROM restaurant WHERE city=$1 ORDER BY ${sortOption} LIMIT $2 OFFSET $3;`,
        [query.city, pageSize, skip]
      );
      ...
      let searchQueryRes;
      if (searchQuery != "") {
        searchQueryRes = await pool.query(
          `SELECT * FROM restaurant WHERE city=$1 AND (restaurantname = $2 OR EXISTS (SELECT 1 FROM unnest(cuisines) AS c WHERE c = $2)) ORDER BY ${sortOption};`,
